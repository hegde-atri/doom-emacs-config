  ...
  (save-excursion
    ...
    (let ((line (thing-at-point 'line t)))
      ...
      (when (and (string-match "^\\s-*\\[" line)
                 (not (string-match "^\\s-*\\[.*dependencies" line)))
        ...
        (cond
         ((string-match "^\\s-*\\([a-zA-Z0-9_-]+\\)\\s-*=\\s-*\"\\([^\"]+\\)\"" line)
          (let ((crate (match-string 1 line))
                ...
                (version-end (+ (line-beginning-position) (match-end 2))))
            (list :crate crate
                  :version (crates--parse-version-string version)
                  :version-end version-end)))
         ((string-match "^\\s-*\\([a-zA-Z0-9_-]+\\)\\s-*=\\s-*{" line)
          (let ((crate (match-string 1 line)))
            (when (string-match "version\\s-*=\\s-*\"\\([^\"]+\\)\"" line)
              (let ((version (match-string 1 line))
                    (version-end (+ (line-beginning-position) (match-end 1))))
                (list :crate crate
                      :version (crates--parse-version-string version)
                      :version-end version-end))))))))))
...
CALLBACK is called with the version string or nil on error."
  (let ((cached (gethash crate crates--version-cache))
    ...
    (if cached
      ...
      (let ((url (format "https://crates.io/api/v1/crates/%s"
                   (url-hexify-string crate))))
...
(defun crates--handle-response (_status crate callback buffer version-cache pending-requests)
  "Handle the HTTP response for crate version fetch.
_STATUS is the request status, CRATE is the crate name,
...
(defun crates--create-overlay (version latest &optional position)
...
If FORCE-FETCH is non-nil, fetch the latest version even if already visited."
  (let* ((crate (plist-get dep :crate))
         ...
         (version-end (plist-get dep :version-end))
         (buffer (current-buffer))
          ...
          (lambda (latest)
            (when (and latest (buffer-live-p buffer))
              (with-current-buffer buffer
                (save-excursion
               (goto-char pos)
               (crates--create-overlay version latest version-end)))))))
